...
export default ()=>{
    const oauthNaver = e => {
        e.preventDefault()
        window.location.href = process.env.REACT_APP_URL+"/oauth2/authorization/naver?redirect_uri=http://localhost:3000/oauth2/redirect"
        }
    const oauthKakao = e => {
        e.preventDefault()
        window.location.href = process.env.REACT_APP_URL+"/oauth2/authorization/kakao?redirect_uri=http://localhost:3000/oauth2/redirect"
        }
    
    const oauthGoogle = e => {
        e.preventDefault()
        window.location.href = process.env.REACT_APP_URL+"/oauth2/authorization/google?redirect_uri=http://localhost:3000/oauth2/redirect"
        }
